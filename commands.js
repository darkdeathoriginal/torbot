...
module.exports = (async () => {
  ...
  const downloadTorrent = {
    ...
    callback: async (m, match, obj) => {
      ...
      const handleTorrent = async (torrent,clear=false) => {
        ...
        torrent.on("download", async(bytes) => {
          ...
          const formatedTime = torrent.timeRemaining < 0 ? "calculating..." : timeInSec<60 ? `${timeInSec.toFixed(0)}s` : timeInSec<3600 ? `${(timeInSec/60).toFixed(0)}m` :timeInSec<3600*24? `${(timeInSec/3600).toFixed(0)}h`: `${(timeInSec/3600/24).toFixed(0)}d`;
      ...
      if(torrent){
        return await handleTorrent(torrent);
      }
      client.add(magnet, { path },handleTorrent);
    },
  };

  return [startCommand, downloadTorrent];
})();
