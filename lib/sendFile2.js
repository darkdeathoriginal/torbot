...
async function uploadFile(client, fileParams) {
  ...
  console.log(partCount,size/partSize);
  ...
  if (isBuffer(file.buffer) || file.path) {
    ...
    for (let i = 0; i < partCount; i += workers) {
      ...
      for (let j = i; j < end; j++) {
        const bytes = Buffer.from(
          await file.buffer.arrayBuffer({
            start: j * partSize,
            end: Math.min((j + 1) * partSize, size),
          })
        );
    ...
    const pushPart = async (jMemo, bytesMemo) => {
      while (true) {
        let sender;
        try {
          // We always upload from the DC we are in
    ...
    for await (const chunk of file.buffer) {
      downloaded = Buffer.concat([downloaded, Buffer.from(chunk)]);

      while (downloaded.length >= partSize) {
        const part = downloaded.slice(0, partSize);
        downloaded = downloaded.slice(partSize);

        sendingParts.push(pushPart(i, part));
    ...
    if (downloaded.length > 0) {
      console.log(downloaded.length, partSize, i, partCount);
      sendingParts.push(pushPart(i, downloaded));
      i++; // Increment i for the last part
  }
    await Promise.all(sendingParts);
  }
