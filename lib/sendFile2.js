...
async function uploadFile(client, fileParams) {
  ...
  if (isBuffer(file.buffer) || file.path) {
    ...
    async function arrayBuffer (opts) {
      for await (const chunk of file.buffer[Symbol.asyncIterator]({size:partSize})) {
        console.log(chunk.length == partSize);
      }
    }
  // arrayBuffer()
   ...
   try {
     for (let i = 0; i < partCount; i++) {
       ...
       const end = Math.min( partSize*(i+1), size ) -1
       const chunks = []
       for await (const chunk of file.buffer[Symbol.asyncIterator]({start,end})) {
        chunks.push(chunk);
       }
       sendingParts.push(pushPart(i, Buffer.concat(chunks)));
   } 
   } catch (error) {
